...
class plotmog2(plotroc) :
    ...
    def __init__(self, indexes=None, internal_range=None, one_value=None, title=None, label=None):
        ...
        self.title = title
        if title == None:
            self.title = 'ROC Curve MOG2 Algorithm'
    ...
    def plot1(self):
        self._generic_plot(self.name_0, self.name_1, 'MOG2_TPR_FPR1.png')
 
    def plot2(self):
        self._generic_plot(self.name_1, self.name_0, 'MOG2_TPR_FPR2.png')

    def plot3(self):
        self._plot_range(self.name_0, self.name_1 , 'MOG2_TPR_FPR3.png')

    def plot4(self):
        self._plot_one_line(self.name_0, self.name_1 , 'MOG2_TPR_FPR4.png', self.parameter_value)

    def plot5(self):
        self._plot_mean_and_median('MOG2_TPR_FPR5.png', self.parameter_value)

    def plot6(self):
        self._plot_mean_and_median_same_graph('MOG2_TPR_FPR6.png', self.parameter_value)
...
if __name__ == '__main__':
    ...
    if options.title == None:
        if os.path.exists("mog2_labels.txt"):
            f = open('mog2_labels.txt', 'r')
            lines = f.readlines()
            f.close()
            ...
            # Creating file
            title = "ROC Curve MOG2 Algorithm"
            text  = "MuHAVI-MAS"
            seq   = "<Sequence Name>"
            ...
            val   = '0.001'
            f     = open("mog2_labels.txt", 'w')
            line="1;%s ;%s\n%s ; %s ; %s ; %s" % (title,text, seq, cols, r4nge, val)
            print >> f,line
            f.close()
    ...
    if options.value != None :
        # Save them in file
        val   = options.value
        update_file('mog2_labels.txt',5,val)

    if options.range != None :
        # Save them in file
        r4nge   = options.range
        update_file('mog2_labels.txt',4,r4nge)

    if options.columns != None :
        # Save them in file
        cols   = options.columns
        update_file('mog2_labels.txt',3,cols)

    n = plotmog2(cols,r4nge, val, title, text)
    n.load(options.file)
    n.plot1()
    n.plot2()
    n.plot3()
