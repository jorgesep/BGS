...
class loadfile:
    ...
    def load(self,filename):
        ...
        data       = [l0.split() for l0 in lines[1:] if len(l0.split()) > 1 and l0[0] != '#']
...
class mergeplots :
    ...
    def __init__(self, files, algorithm, columns, values, title=None, label=None, xlimits=None, ylimits=None):
        ...
        # Preparing limits x,y in display graphs.
        self.x0=None
        self.x1=None
        if xlimits != None:
            self.x0 = xlimits.split(',')[0]
            self.x1 = xlimits.split(',')[1]

        self.y0=None
        self.y1=None
        if ylimits != None:
            self.y0 = ylimits.split(',')[0]
            self.y1 = ylimits.split(',')[1]
    ...
    def plot1(self):
        ...
        for i in range(len(self.files)):
            ...
            # preparation of legend box in plot.
            label = '%s %s' % (str(self.algorithm[i]), str(value))
        ...
        if self.x0 != None and self.x1 != None:
            plt.xlim((float(self.x0),float(self.x1)))
        if self.y0 != None and self.y1 != None:
            plt.ylim((float(self.y0),float(self.y1)))
        plt.savefig(filename)
...
if __name__ == '__main__':
    ...
    parser.add_option ("-x", "--xlimit",dest = "_xlim",
                       type = "string",
                       default = None,
                       help = "Select xlimit to display : -x '0,0.5'")
    parser.add_option ("-y", "--ylimit",dest = "_ylim",
                       type = "string",
                       default = None,
                       help = "Select ylimit to display : -y '0,0.5'")
    (options, args) = parser.parse_args ()

    if options.file == None :
        print "Select single file containing all summary files of roc measures"
        parser.print_help()
        sys.exit()
    ...
    if options.label != None :
        ...
        if os.path.exists('mergeplot_labels.txt') :
            ...
            value_0 = values[0]
            label = ''
            label += "%s=%s" % (columns[0].split(',')[0], values[0]) + "\n"
            for i in values.keys() : 
                if values[i] != value_0 :
                    label += "%s=%s" % (columns[i].split(',')[0], values[i]) + "\n"
            label += "%s=%s" % (columns[0].split(',')[1], second[0]) + "\n"
            label += 'MuHAVI ' + sequence[0]
    ...
    n = mergeplots(files, algorithm, columns, values, title, label, options._xlim, options._ylim) 
    n.load()
    n.plot1()
